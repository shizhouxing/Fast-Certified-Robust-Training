...
# python cifar_training.py --model=cnn_7layer_bn2 
...
parser.add_argument("--data", type=str, default="CIFAR", choices=["MNIST", "CIFAR", "TINYIMAGENET"], help='dataset')
parser.add_argument("--seed", type=int, default=100, help='random seed')
parser.add_argument("--eps", type=float, default=0.0, help='Target training epsilon')
...
if args.data == 'MNIST' and args.eps == 0.0:
    args.eps=0.4
elif args.data == 'CIFAR' and args.eps == 0.0:
    args.eps = 0.03137
elif args.data == 'TINYIMAGENET' and args.eps == 0.0:
    args.eps = 0.003921
print('Setting eps to {}'.format(args.eps))
...
if args.verify:
    logger = Logger(open(os.path.join(args.dir, exp_name + '_test.log'), "w"))
else:
    logger = Logger(open(os.path.join(args.dir, 'train.log'), "w"))
...
def main(args):
    ...
    if args.data == 'MNIST':
        ...
        in_ch, in_dim = 3, 64
    model_ori = eval(args.model)(in_ch=in_ch, in_dim=in_dim).to(args.device)
    epoch = 0
    if args.load:
    ...
    if args.data == 'MNIST':
        ...
        dummy_input = torch.randn(2, 3, 64, 64).to(args.device)
    ...
    model_loss = BoundedModule(CrossEntropyWrapper(model_ori), (dummy_input, torch.zeros(1, dtype=torch.long, device=args.device)),
        ...
        for t in range(epoch + 1, args.num_epochs+1):
            ...
            with torch.no_grad():
                if t > int(eps_scheduler.params['start']) + int(eps_scheduler.params['length']):
                    m = Train(model_loss, t, test_data, FixedScheduler(args.eps), norm, False, None, 'IBP', loss_fusion=False,
            ...
            if t < int(eps_scheduler.params['start']):
                ...
                if current_err < best_err:
                    ...
                    torch.save(save_dict, os.path.join(args.dir, exp_name + '_best_' + str(best_err)[:6]))
                else:
                    torch.save(save_dict, os.path.join(args.dir, exp_name))
            else:
                torch.save(save_dict, os.path.join(args.dir, exp_name))
            torch.cuda.empty_cache()
